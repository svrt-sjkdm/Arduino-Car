...
void evadeObstaculo() {
  ...
  lcd.print("Detectado");
  PORTB = B000000; 
  servo.write(20);           // Girar servo a la izquierda
  distancia = medirDistancia();
  if(distancia <= maxDis) {  // Encontro obstaculo a la izq
    servo.write(160);        // Girar servo a la derecha
    distancia = medirDistancia();
    if(distancia <= maxDis) { // Encontro obstaculo a la der
      for(int i = 0; i <= 200; i++) {
        PORTB = B101000;     // Hacia atras
        printDirection("Atras");
      } 
    }
    else {
